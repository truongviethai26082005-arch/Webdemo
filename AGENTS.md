...
- **Sale (định hướng):** Sẽ là **chủ sở hữu duy nhất** của tính năng tuyển sinh/admissions (leads, trials, conversions...). Admin **không** thao tác chi tiết tính năng này, chỉ xem **báo cáo tổng hợp** (tự động tổng hợp, hoặc khi Sale bấm "gửi báo cáo").
  ...
  - **Ngoại lệ đã xác nhận (2026-09-14):** Sale được quyền **quản lý đầy đủ tài khoản đăng nhập của học sinh** (xem/tạo/đổi mật khẩu) dù `students`/tài khoản không phải "lãnh địa" gốc của Sale — lý do: Sale là người trực tiếp làm việc/hỗ trợ học sinh & phụ huynh hằng ngày. Xem chi tiết 3 hàm được cấp quyền ở Mục 7 và `docs/context-sale.md`.
...
- [x] **`createStudent()` (`lib/actions/students.ts`) — vá trước khi bàn giao phân hệ Sale (2026-09-14, phát hiện khi audit tính năng Tuyển sinh mock):** đây là hàm dùng chung mà Sale BẮT BUỘC phải tái sử dụng (Mục 9). Trước đó: (a) tự bịa `initial_sessions = 12` khi thiếu dữ liệu (vi phạm Mục 11.1); (b) khi ghi `enrollments` thất bại, chỉ `console.error` rồi vẫn trả `success:true` — tạo học sinh thật trong DB nhưng không có lớp/buổi học nào (học sinh "mồ côi"). Đã sửa: bắt buộc số buổi hợp lệ khi có `class_id`, và tự hoàn tác (xóa) bản ghi `students` vừa tạo nếu ghi danh thất bại thay vì báo thành công giả. Chi tiết đầy đủ + bối cảnh phát hiện ở `docs/context-sale.md`.
- [x] **`submitLead()` (`lib/actions/leads.ts`) — đổi tên bảng đích từ `"leads"` sang `"landing_page_leads"` (2026-09-14):** tránh trùng tên với bảng `leads` mà phân hệ Sale sắp tạo cho phễu Tuyển sinh phụ huynh (2 nghiệp vụ hoàn toàn khác nhau — form này là lead B2B "trung tâm muốn mua phần mềm"). Nếu không đổi, ngay khi Sale tạo bảng `leads` thật, hàm landing-page này sẽ âm thầm ghi dữ liệu sai định dạng vào đó.
- [x] **`createAccountByAdmin()` (`lib/actions/auth.ts`) — xóa mật khẩu mặc định `"password123"` (2026-09-14, phát hiện ở đợt rà soát cuối trước bàn giao):** hàm này AGENTS.md Mục 9 nói Sale phải tái sử dụng để tự tạo tài khoản học sinh sau này — nếu 1 lời gọi nào quên truyền `password`, trước đây sẽ âm thầm tạo tài khoản thật với mật khẩu đoán được `"password123"`. Đã sửa: bắt buộc truyền mật khẩu tối thiểu 8 ký tự, không có mặc định nào — thiếu thì báo lỗi rõ ràng thay vì tự bịa.
- [ ] **Dọn DB — hàm trigger mồ côi + hardening `search_path` (2026-09-14, phát hiện qua Supabase Security Advisor):** `handle_attendance_balance()`, `handle_attendance_deduction()`, `handle_invoice_paid()` không còn gắn trigger nào (đã xác minh qua `pg_trigger`) nhưng vẫn tồn tại trong DB dưới dạng SECURITY DEFINER, lộ qua REST RPC cho anon/authenticated (không khai thác được thực tế vì là hàm kiểu trigger, nhưng nên dọn sạch). SQL đã viết sẵn ở `supabase/migrations/20260914_cleanup_orphaned_trigger_functions_and_harden_search_path.sql` — **CHƯA CHẠY, cần chủ dự án tự chạy trên Supabase Dashboard** (Claude không có quyền ghi DB trực tiếp, đúng thông lệ cả phiên). Đồng thời khuyến nghị bật "Leaked Password Protection" trong Supabase Dashboard → Authentication → Policies (advisor cảnh báo đang tắt) — thao tác 1 click, không cần SQL.
- [x] **Hoàn thiện quản lý tài khoản học sinh + mở quyền cho Sale (2026-09-14):** trang `/admin/accounts` trước đó chỉ hiện tên học sinh đã có tài khoản dạng badge tĩnh, không xem được email, không đổi được mật khẩu. Đã thêm `getStudentAccountsOverview()` (`lib/actions/accounts.ts`, admin+sale) và `resetStudentPassword()` (`lib/actions/students.ts`, admin+sale), gắn UI thật (bảng + dialog đổi mật khẩu) ở `accounts-client.tsx`. Đồng thời **nới quyền `createAccountByAdmin()`** để Sale gọi được — nhưng CHỈ khi tạo tài khoản `role="student"`, chặn cứng ở tầng server (Sale không bao giờ tạo được tài khoản Admin/Teacher/Sale qua hàm này dù truyền gì). Quyết định: Sale được "quản lý đầy đủ" (không chỉ xem) tài khoản học sinh vì là người trực tiếp hỗ trợ học sinh/phụ huynh — xác nhận trực tiếp với chủ dự án. Chi tiết cho người code Sale ở `docs/context-sale.md`.
- [x] **`getCenterBankSettings()` (`lib/actions/settings.ts`) — hết bịa dữ liệu (2026-09-22):** đổi kiểu trả về sang `CenterBankSettings | null`, không còn tự thay bằng 1 tài khoản ngân hàng khác khi lỗi/thiếu dữ liệu. Mọi nơi tạo mã QR thu tiền (Sale: `conversion-checkout-modal.tsx`, `add-class-to-student-dialog.tsx`) đã cập nhật để không tạo QR khi tài khoản rỗng, hiện rõ "Chưa cấu hình tài khoản ngân hàng" thay vào đó. Chi tiết: `docs/context-sale.md`, `docs/context-handoff.md` Mục 18.
...
- [x] **3 trang Teacher (`assignments`/`grading`/`resources`) từng 100% mock — nay nối dữ liệu thật (2026-09-22):** file mới `lib/actions/assignments.ts`/`materials.ts`. Đây là mắt xích Teacher→Student còn thiếu — trước đó các trang Student tương ứng dù code đúng vẫn luôn trống. Chi tiết: `docs/context-teacher.md`.
- [x] **Bảng `student_feedbacks` chưa từng tồn tại trên DB thật dù code đã viết từ trước (2026-09-22, lỗ hổng nghiêm trọng phát hiện khi rà soát) — đã tạo bảng thật + vá lỗi nuốt exception:** mọi phản hồi học sinh gửi trước đây đều thất bại thầm lặng (báo thành công giả). Sale giờ đọc + trả lời được qua `/sale/feedback`. Chi tiết: `docs/context-student.md`, `docs/context-sale.md`.
- [x] **Admin lần đầu xem được báo cáo tổng hợp Tuyển sinh (2026-09-22):** trang mới `/admin/admissions-report`, chỉ đọc, tái dùng 100% Server Action Sale đã có. Chi tiết: `docs/context-admin.md`.

**Nghiệp vụ Admin/Teacher còn treo (cập nhật 2026-09-22) — KHÔNG chặn code song song Sale/Student, để người phụ trách Admin/Teacher xử lý tiếp khi quay lại phân hệ đó. Chi tiết đầy đủ ở `docs/context-handoff.md` Mục 15.2:**
- [ ] 3 dialog vẫn còn bỏ qua `result.error` từ Server Action, ghi dữ liệu giả vào store khi thất bại: `class-dialog.tsx`, `teacher-dialog.tsx` (còn ghi email giả), `add-student-dialog.tsx` (tab "học sinh đã có sẵn" ghi optimistic trước khi gọi server, không rollback khi lỗi). `student-dialog.tsx` đã tự sửa một phần (có check `result.error`) nhưng vẫn còn fallback "12 buổi" khi để trống ô số buổi. **Xem quy tắc bắt buộc mới ở mục 3 (Server Actions) để không lặp lại lỗi này khi viết dialog cho Sale/Student.** Đây là file riêng của Admin (Nhóm 3, không ảnh hưởng phân hệ khác) nên chưa vá đợt 2026-09-22.
...
- [x] ~~`getTeacherPersonalEarnings` và `getTeacherPayroll` tính thu nhập giáo viên theo 2 công thức khác nhau khi có dạy thay~~ — đã thống nhất lại 2026-09-22, xem mục trên.
...
**Hạ tầng/quy trình:**
- [x] `app/sale/layout.tsx`, `app/student/layout.tsx` — placeholder ban đầu (2026-09-12) hoàn toàn KHÔNG có nút đăng xuất. Vì `proxy.ts` chặn đúng thiết kế (Sale/Student không được sang `/admin`, `/teacher`, phân hệ khác, và tự đá về lại trang nhà nếu vào `/login` khi đã đăng nhập), tài khoản đăng nhập vào 2 phân hệ này bị **kẹt cứng, không thoát ra được** — phát hiện thật ngày 2026-09-13 khi test tài khoản Sale vừa tạo. Đã thêm header tối thiểu (tên hệ thống + nút "Đăng xuất" dùng lại `signOut()` có sẵn) cho cả 2. **Bài học cho ai xây tính năng thật thay placeholder này: bất kỳ layout nào gate theo role đều BẮT BUỘC phải có đường thoát (đăng xuất), không chỉ có nội dung chính.**
