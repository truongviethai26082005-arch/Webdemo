...
// A3: Cho Admin xem lại email đăng nhập của 1 giáo viên (email chỉ nằm ở auth.users,
// không có trong bảng profiles) và đặt lại mật khẩu hộ khi giáo viên quên.
export async function getTeacherEmail(teacherId: string) {
    const guard = await requireRole(["admin"]);
    if (!guard.authorized) return null;

    const adminClient = createAdminClient();
    const { data, error } = await adminClient.auth.admin.getUserById(teacherId);
...
export async function getTeacherPersonalEarnings(teacherId?: string, month?: number, year?: number) {
    ...
    if (currentProfile.role === "teacher") {
        // Bắt buộc ghi đè teacherId bằng user.id của chính họ để chống IDOR
        currentTeacherId = user.id;
    } else if (currentProfile.role === "admin") {
        // Admin có thể xem theo teacherId được truyền vào, hoặc xem của chính mình nếu không truyền
        currentTeacherId = teacherId || user.id;
    } else {
        // Các vai trò khác không có quyền xem thu nhập giáo viên
        return null;
    }
